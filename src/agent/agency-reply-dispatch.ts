...
export function isAgencyReplyTransportEnabled(): boolean {
  try {
    const config = env();
    return config.AGENCY_REPLY_TRANSPORT_ENABLED === true
...
function validateInput(input: AgencyReplyDispatchInput): string | undefined {
  ...
  if (!Number.isSafeInteger(input.segment) || input.segment < 0 || input.segment > 999) return 'invalid_reply_segment';
  if (typeof input.text !== 'string' || !input.text.trim()) return 'empty_reply_text';
 ...
 */
export async function dispatchReplyViaAgency(input: AgencyReplyDispatchInput): Promise<AgencyReplyDispatchResult> {
  if (!isAgencyReplyTransportEnabled()) {
    return { attempted: false, accepted: false, reason: 'agency_reply_transport_disabled' };
  }
  ...
  let dispatched;
  try {
    dispatched = await dispatchAgencyRun(created.run.id, createTelegramAgencyDeliveryAdapters());
  } catch {
    return {
      attempted: true,
      accepted: false,
  ...
  if (!dispatched.ok || messageId === undefined) {
    return {
      ...
      accepted: false,
      agencyRunId: run.id,
      reason: messageId === undefined ? runReason(run, dispatched.reason) : (dispatched.reason ?? 'agency_dispatch_rejected'),
    };
  }
  return {
    attempted: true,
