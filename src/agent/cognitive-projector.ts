...
function factProperties(event: CognitiveEvent): Record<string, string> {
  ...
  for (const [key, raw] of Object.entries(value).slice(0, 20)) {
    if (!/^[^\u0000-\u001f]{1,80}$/.test(key) || typeof raw !== 'string') continue;
...
function hasWorldRevisionForEvent(event: CognitiveEvent): boolean {
  try {
    const db = getDb();
    const table = db.prepare("SELECT 1 FROM sqlite_master WHERE type = 'table' AND name = 'world_entity_revisions'").get();
    if (!table) return false;
    return Boolean(db.prepare('SELECT 1 FROM world_entity_revisions WHERE source_event_id = ? LIMIT 1').get(event.id));
  } catch {
    return false;
  }
}
...
function isSource(event: CognitiveEvent, ...sources: CognitiveEvent['source'][]): boolean {
...
function projectSuccessfulToolCallback(event: CognitiveEvent): number {
  if (event.type !== 'task_observation' || event.source !== 'tool' || !event.taskId || !event.chatId) return 0;
  if (factText(event, 'kind') !== 'tool_finished' || factText(event, 'errorCode')) return 0;
  ...
  try {
    ...
    ).all(event.chatId, event.taskId) as Array<{ id?: number; source_event_ids?: unknown }>;
    for (const row of rows) {
      const sourceId = sourceEventIds(row.source_event_ids)[0];
      if (!sourceId) continue;
      const source = getCognitiveEvent(sourceId);
      if (!source || source.type !== 'tool_failure' || source.taskId !== event.taskId || source.chatId !== event.chatId) continue;
      if (factText(source, 'kind') !== 'tool_finished' || factText(source, 'toolName') !== toolName) continue;
      if (!factText(source, 'errorCode') || factText(source, 'invocationId') === invocationId) continue;
      if (row.id && resolveDebtWithEvidence({
        id: row.id,
        resolution: `工具 ${toolName} 后续调用已成功返回`,
        resolutionEventId: event.id,
        scope: { visibility: 'task', chatId: event.chatId, taskId: event.taskId },
      })) return 1;
    }
  } catch (err) {
    logger.debug({ err, eventId: event.id }, 'successful tool callback projection failed');
  }
  return 0;
}

function projectWorldChange(event: CognitiveEvent): boolean {
  if (event.type !== 'world_change' || event.visibility !== 'chat' || !event.chatId) return false;
  // Model-authored world facts are hypotheses, not host-observable changes.
  if (event.source !== 'host' && event.source !== 'tool' && event.source !== 'scheduler') return false;
  const name = factText(event, 'entityName') || factText(event, 'name');
  const kind = factText(event, 'entityKind') || factText(event, 'kind');
  if (!name || !['person', 'project', 'topic', 'place'].includes(kind)) return false;
  ...
  const confidence = typeof confidenceRaw === 'number' && Number.isFinite(confidenceRaw)
    ? Math.min(1, Math.max(0, confidenceRaw)) : 0.5;
  const expiresRaw = event.fact['expiresAt'];
  const expiresAt = typeof expiresRaw === 'number' && Number.isSafeInteger(expiresRaw) && expiresRaw > 0
    ? expiresRaw : undefined;
  return upsertEntity(
    name,
    kind as 'person' | 'project' | 'topic' | 'place',
    properties,
    event.chatId,
    { visibility: 'chat', chatId: event.chatId },
    { sourceEventId: event.id, confidence, ...(expiresAt !== undefined ? { expiresAt } : {}) },
  ) !== null;
}
...
function hasDebtForEvent(event: CognitiveEvent, kind: string): boolean {
  ...
  try {
    const rows = getDb()
      .prepare('SELECT source_event_ids FROM cognitive_debts WHERE chat_id = ? AND kind = ? ORDER BY id DESC LIMIT 50')
...
function debtSpec(event: CognitiveEvent): {
  ...
  if (event.type === 'user_correction' && !isSource(event, 'telegram', 'import')) return null;
  if (event.type === 'user_stop' && !isSource(event, 'telegram', 'host', 'import')) return null;
  if (event.type === 'user_goal_change' && !isSource(event, 'telegram', 'host', 'import')) return null;
  if (event.type === 'tool_failure' && !isSource(event, 'tool', 'host', 'scheduler', 'import')) return null;
  if (event.type === 'task_observation' && !isSource(event, 'host', 'tool', 'scheduler', 'import')) return null;
  if (event.type === 'world_change' && !isSource(event, 'host', 'tool', 'scheduler')) return null;
  const detail = factText(event, 'toolName') || factText(event, 'errorCode') || factText(event, 'reason');
  if (event.type === 'user_correction') {
    return {
      kind: 'correction',
      statement: detail ? `用户纠正了先前判断，需要重新核实（${detail}）` : '用户纠正了先前判断，需要重新核实',
      priority: 8,
      confidence: 0.9,
    };
  }
  if (event.type === 'tool_failure') {
    if (factText(event, 'kind') === 'task_failed') {
      if (factText(event, 'resultSummary') === 'failed_user_stopped' || factText(event, 'resultSummary') === 'user_stopped') {
        return null;
      }
      return {
        kind: 'unfinished_task',
        statement: detail ? `任务未完成，需要重新处理（${detail}）` : '任务未完成，需要重新处理',
        priority: 8,
        confidence: 0.9,
      };
    }
    return {
      kind: 'uncertainty',
      statement: detail ? `工具结果未取得，需要重新核实（${detail}）` : '工具结果未取得，需要重新核实',
      priority: 7,
      confidence: 0.8,
    };
  }
  ...
  if (event.type === 'task_observation') {
    ...
    if (kind === 'task_failed' || kind === 'task_waiting_user' || kind === 'task_queued') {
      return {
        kind: 'unfinished_task',
        statement: kind === 'task_waiting_user' ? '任务等待用户输入，尚未完成' : '任务尚未得到外部验收的完成结果',
...
function projectPredictionFeedback(event: CognitiveEvent): boolean {
  if (event.type !== 'user_reaction' && event.type !== 'user_followup') return false;
...
function projectDebtResolution(event: CognitiveEvent): number {
  if (!isSource(event, 'telegram', 'host', 'tool', 'scheduler', 'import')) return 0;
  const resolution = factText(event, 'resolution') || factText(event, 'resolutionEvidence');
  ...
  const verifiedCompletion = observationKind === 'task_completed'
...
export function projectCognitiveEvent(
  ...
  try {
    ...
    const debtsCreated = options.createDebts === false ? 0 : createDebtForEvent(event);
    ...
    return {
      ...
      ignored: !predictionFeedbackSeen && debtsCreated === 0 && debtsResolved === 0 && worldChangeProjected === 0 && toolCallbackProjected === 0,
    };
  } catch (error) {
    logger.warn({ err: error, eventId: event.id, type: event.type }, 'cognitive event projection failed');
    return { eventId: event.id, predictionFeedbackSeen: false, debtsCreated: 0, debtsResolved: 0, worldChangeProjected: 0, toolCallbackProjected: 0, ignored: true };
  }
}
