...
function transportEnabled(): boolean {
  try {
    const config = env();
    return config.AGENCY_CODEACT_TRANSPORT_ENABLED === true
 ...
 */
export async function dispatchCodeActTaskViaAgency(task: DispatchTask): Promise<AgencyCodeActDispatchResult> {
  if (!transportEnabled()) {
    return { attempted: false, accepted: false, reason: 'agency_codeact_transport_disabled' };
  }
  if (!task || typeof task.id !== 'string' || !task.id.trim() || task.id.trim().length > 120) {
  ...
  if (
    ...
    return { attempted: true, accepted: false, reason: 'invalid_cognitive_anchor_event_id' };
  }

  const envelope = createAgencyEnvelope({
    action: { type: 'act', goal: task.contentDirection.trim().slice(0, 2000), taskId: task.id.trim() },
    ...
    causationId: task.cognitiveAnchorEventId?.trim() || `task:${task.id.trim()}`,
  ...
  let dispatched;
  try {
    dispatched = await dispatchAgencyRun(created.run.id, createCodeActAgencyActAdapters(task));
  } catch {
    return {
      attempted: true,
      accepted: false,
  ...
  if (acceptedId !== task.id.trim()) {
    return {
      ...
      accepted: false,
      agencyRunId: run.id,
      reason: acceptedId ? 'agency_task_id_mismatch' : runReason(run, dispatched.reason),
    };
  }
  return {
    attempted: true,
