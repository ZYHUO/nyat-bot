...
import { getProfileSections, getUserProfilePrompt } from '../tracking/user-profile.js';
import { getGroupNorms } from './group-norms.js';
import { getRelationship, getRelationshipAt } from '../tracking/relationship.js';
import { findEntities, listEntityRevisions, type WorldEntity } from './world-state.js';
...
type NormalizedWorldProjectionBudget = Required<Omit<WorldProjectionBudget, 'asOf'>> & { asOf?: number };

function normalizeBudget(input: WorldProjectionBudget): NormalizedWorldProjectionBudget {
  const asOf = Number.isSafeInteger(input.asOf) && (input.asOf as number) > 0 ? input.asOf : undefined;
  return {
...
function validChatScope(scope: CognitiveScope): boolean {
  if (!scope || scope.visibility === 'global' || !Number.isSafeInteger(scope.chatId) || scope.chatId === 0) return false;
  try {
    scopeKey(scope);
    return true;
  } catch {
...
function profileMeta(chatId: number, userId: number, asOf?: number): { updatedAt: number; stale: boolean } {
  try {
    ...
    const params = asOf === undefined ? [chatId, userId] : [chatId, userId, asOf];
    ...
    return {
      updatedAt: Number.isSafeInteger(updatedAt) && updatedAt > 0 ? updatedAt : nowSec(),
...
function profileStatement(chatId: number, userId: number, asOf?: number): string {
  let profile = '';
  try {
  ...
  try {
    ...
    return sections
      .slice(0, 8)
      .map((section) => {
        const bullets = section.bullets.slice(0, 3).map((bullet) => safeText(bullet, 120)).filter(Boolean);
        return bullets.length ? `${safeText(section.section_name, 50)}: ${bullets.join('；')}` : '';
      })
      .filter(Boolean)
      .join('\n')
      .slice(0, 480);
...
function buildSelf(limit: number, uncertainties: string[], asOf?: number): ProjectionHypothesis[] {
  try {
    ...
    for (const note of getActiveSelfNotes(limit, asOf)) {
      ...
      if (!statement || !Number.isSafeInteger(note.id) || note.id <= 0) continue;
      hypotheses.push({
          id: `self:${note.id}`,
          kind: 'self',
          subject: 'bot',
...
function buildPerson(scope: CognitiveScope, limit: number, uncertainties: string[], asOf?: number): ProjectionHypothesis[] {
  if (!Number.isSafeInteger(scope.chatId) || scope.chatId === 0 || !Number.isSafeInteger(scope.userId) || scope.userId === 0) return [];
  ...
  if (asOf === undefined) {
    ...
    try {
      ...
      if (relationship === null) {
        uncertainties.push(`用户关系统计缺少历史 revision，事件锚点不读取当前关系（uid:${userId}@chat:${chatId}）。`);
      ...
      uncertainties.push(`用户关系统计暂时不可用，事件锚点不读取当前关系（uid:${userId}@chat:${chatId}）。`);
    }
  }
  const statement = [profile, relationshipLine].filter(Boolean).join('\n').slice(0, 480);
  if (!statement) return [];
  ...
  if (stale) uncertainties.push(`用户画像可能已过期（uid:${userId}@chat:${chatId}）。`);
...
function buildGroup(scope: CognitiveScope, limit: number, uncertainties: string[], asOf?: number): ProjectionHypothesis[] {
  if (!Number.isSafeInteger(scope.chatId) || typeof scope.chatId !== 'number' || scope.chatId >= 0) return [];
  const chatId = scope.chatId;
  try {
    const norms = getGroupNorms(chatId, asOf);
    const statements = norms?.norms.map((norm) => safeText(norm, 100)).filter(Boolean) ?? [];
    if (!norms || !statements.length) return [];
    const updatedAt = Number.isSafeInteger(norms.lastUpdatedAt) && norms.lastUpdatedAt > 0 ? norms.lastUpdatedAt : nowSec();
    ...
    const sampleCount = Math.max(0, Number.isFinite(norms.sampleCount) ? norms.sampleCount : 0);
    const hypothesis: ProjectionHypothesis = {
      id: `group:${chatId}`,
      kind: 'group',
      subject: `chat:${chatId}`,
...
function entityScope(entity: WorldEntity, chatId: number): string | null {
  ...
  if (persisted && persisted !== 'legacy') {
    if (persisted === 'global' || persisted === `chat:${chatId}`) return persisted;
    return null;
  }
  if (entity.sourceChatId === null || entity.sourceChatId === undefined) return 'global';
  return entity.sourceChatId === chatId ? `chat:${chatId}` : null;
}

function buildWorld(scope: CognitiveScope, limit: number, uncertainties: string[], asOf?: number): { hypotheses: ProjectionHypothesis[]; entities: WorldEntity[] } {
  if (!Number.isSafeInteger(scope.chatId) || scope.chatId === 0) return { hypotheses: [], entities: [] };
  const chatId = scope.chatId as number;
  try {
    const candidates = findEntities('', undefined, limit, scope, { ...(asOf === undefined ? {} : { asOf }) });
    ...
    for (const currentEntity of candidates) {
      ...
      if (asOf !== undefined && entity.lastUpdatedAt > asOf) {
        const revisionScope = scoped === 'global'
          ? { visibility: 'global' as const }
          : { visibility: 'chat' as const, chatId };
        const revision = listEntityRevisions(entity.id, 100, revisionScope)
        ...
        if (revision) {
          ...
          uncertainties.push(`实体 ${kind}:${name} 在事件锚点之后才有可见更新，已从历史投影中排除。`);
          continue;
        }
      }
      ...
      const updatedAt = Number.isSafeInteger(entity.lastUpdatedAt) && entity.lastUpdatedAt > 0 ? entity.lastUpdatedAt : now;
      ...
      hypotheses.push({
        ...
        kind: 'world',
        subject: `${kind}:${name}`,
        statement: `${name}（${kind}）${properties.length ? `: ${properties.join(', ')}` : ''}`.slice(0, 500),
        source: entity.sourceEventId ? `world_event:${entity.sourceEventId}` : `world_entities:${entity.id}`,
        scopeKey: scoped,
    ...
    return { hypotheses: hypotheses.slice(0, limit), entities: entities.slice(0, limit) };
  } catch {
...
export function buildScopedWorldProjection(
  ...
  if (!validChatScope(scope)) {
    return {
      ...
      world: [],
      worldEntities: [],
      uncertainties: ['工作区缺少有效 chat scope，已拒绝 Person/Group/World 投影。'],
    };
  }
  const person = buildPerson(scope, budget.maxPerson, uncertainties, budget.asOf);
