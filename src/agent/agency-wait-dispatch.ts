...
export type AgencyWaitSource = 'heart' | 'meta_heart' | 'meta_timing' | 'dispatch_gate' | 'pipeline_gate';
...
export function isAgencyWaitTransportEnabled(): boolean {
  try {
    const config = env();
    return config.AGENCY_WAIT_TRANSPORT_ENABLED === true
...
function runReason(run: AgencyRun | undefined, fallback?: string): string {
  return run?.error?.trim().slice(0, 240)
    || fallback?.trim().slice(0, 240)
    || (run ? `agency_run_${run.status}` : 'agency_run_unavailable');
}

function acceptedWait(run: AgencyRun | undefined): { waitUntil: number; waitJobId?: string } | undefined {
  if (!run || run.status !== 'succeeded' || !run.result || typeof run.result !== 'object' || Array.isArray(run.result)) {
    return undefined;
  }
  const result = run.result as Record<string, unknown>;
  const waitUntil = result['waitUntil'];
  if (typeof waitUntil !== 'number' || !Number.isSafeInteger(waitUntil) || waitUntil <= 0) return undefined;
  const waitJobId = result['waitJobId'];
  if (waitJobId !== undefined && (typeof waitJobId !== 'string' || !waitJobId.trim() || waitJobId.length > 240)) {
    return undefined;
  }
  return {
    waitUntil,
...
function validateInput(input: AgencyWaitDispatchInput): string | undefined {
  ...
  if (!Number.isSafeInteger(input.chatId) || input.chatId === 0) return 'scoped_chat_required';
  if (!Number.isSafeInteger(input.triggerMessageId) || input.triggerMessageId <= 0) return 'invalid_trigger_message_id';
  if (
  ...
  if (!Number.isFinite(input.waitSec) || input.waitSec < 0 || input.waitSec > 24 * 3600) return 'invalid_wait_seconds';
  if (!['heart', 'meta_heart', 'meta_timing', 'dispatch_gate', 'pipeline_gate'].includes(input.source)) {
    return 'invalid_wait_source';
  }
  if (typeof input.reason !== 'string' || !input.reason.trim()) return 'empty_wait_reason';
  if (input.reason.trim().length > 500) return 'wait_reason_too_long';
  if (
    input.obligationId !== undefined
    && (typeof input.obligationId !== 'string' || !input.obligationId.trim() || input.obligationId.trim().length > 120)
...
export async function dispatchWaitViaAgency(input: AgencyWaitDispatchInput): Promise<AgencyWaitDispatchResult> {
  if (!isAgencyWaitTransportEnabled()) {
    return { attempted: false, accepted: false, reason: 'agency_wait_transport_disabled' };
  }
  ...
  if (!envelope.ok || !envelope.envelope) {
    return { attempted: true, accepted: false, reason: envelope.reason ?? 'invalid_agency_envelope' };
  }

  const created = createAgencyRun(envelope.envelope);
  if (!created.ok || !created.run) {
    return { attempted: true, accepted: false, reason: created.reason ?? 'agency_run_unavailable' };
  }
  ...
  try {
    dispatched = await dispatchAgencyRun(
      ...
      createTimingAgencyWaitAdapters({
        ...
        ...(input.triggerUserId !== undefined ? { triggerUserId: input.triggerUserId } : {}),
        ...(input.obligationId !== undefined ? { obligationId: input.obligationId.trim() } : {}),
      }),
    );
  } catch {
    return { attempted: true, accepted: false, agencyRunId: created.run.id, reason: 'agency_dispatch_failed' };
  }
  ...
  if (!dispatched.ok || !wait) {
    return {
      ...
      accepted: false,
      agencyRunId: run.id,
      reason: wait ? (dispatched.reason ?? 'agency_dispatch_rejected') : runReason(run, dispatched.reason),
    };
  }
  return {
    attempted: true,
