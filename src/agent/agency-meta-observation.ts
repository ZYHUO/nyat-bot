...
function observeAdapter(action: AgencyAction): { recorded: true; target: string } {
  return {
    recorded: true,
    target: action.type === 'observe' ? action.target : 'invalid-meta-observation-action',
  };
}
...
export async function recordMetaDispatchObservation(
  ...
  if (!validChatId(input.chatId)) return { ok: false, reason: 'invalid_chat_id' };
  if (!['L0', 'L1', 'L1_CALLBACK', 'L2'].includes(input.layer)) {
    return { ok: false, reason: 'invalid_attention_layer' };
  }
  const quoteMessageIds = (Array.isArray(input.quoteMessageIds) ? input.quoteMessageIds : [])
    .filter(validMessageId)
    .slice(0, 8);
  if (!validAnchor(input.cognitiveAnchorEventId)) {
    return { ok: false, reason: 'invalid_cognitive_anchor_event_id' };
  }
  if (input.targetUserId !== undefined && (!Number.isSafeInteger(input.targetUserId) || input.targetUserId <= 0)) {
    return { ok: false, reason: 'invalid_target_user_id' };
  }

  const taskId = typeof input.taskId === 'string' ? input.taskId.trim().slice(0, 120) || undefined : undefined;
  const relatedQuoteCount = boundedRelatedQuoteCount(input.relatedQuoteCount);
  const cognitiveAnchorEventId = typeof input.cognitiveAnchorEventId === 'string'
  ...
  const stableAnchor = triggerMessageId !== undefined
  ...
  const idempotencyKey = `meta-dispatch:${input.chatId}:${stableAnchor}:${input.layer}`
    + (decision === 'proposed' ? '' : `:${decision}:${decisionReason}`);
  const correlationId = triggerMessageId !== undefined
  ...
  const envelope = createAgencyEnvelope({
    ...
    idempotencyKey,
    correlationId,
    causationId: cognitiveAnchorEventId
  ...
  let dispatched;
  try {
    dispatched = await dispatchAgencyRun(created.run.id, { observe: observeAdapter });
  } catch {
    return {
      ok: false,
