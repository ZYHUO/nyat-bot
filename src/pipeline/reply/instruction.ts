...
const IMPERATIVE_PATTERNS: RegExp[] = [
  // 请求/委托开头
  /^(请|帮我|帮忙|给我|给爷|替我|麻烦你?|你去|你来|你给|快点?|马上|立刻|现在就)/,
  // 动词开头的直接命令
  /^(发|说|讲|写|画|翻译|总结|重复|再说一?遍|复述|背|列出?|报|念|读|告诉|解释|查一?下|搜一?下|算一?下|数一?下|改成|换成|换个|删掉|撤回|重发|补充|继续说?|接着说?|展开)/,
  // 用 X 说/写/回答
  /^用.{1,10}(说|写|回|答|讲|翻译|重复)/,
  // 把字句
  /把.{1,30}(发出来|说出来|改成|换成|翻译|总结|重复|再说|列出来|贴出来|写出来|读出来|发一遍|发给)/,
  // 禁止/停止类
  /^(别再?|不要再?|不许|不准|禁止|停止?|停下|闭嘴|安静|住口|消停)/,
  // 必须/给我 + 动词
