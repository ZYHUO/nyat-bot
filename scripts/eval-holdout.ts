...
export function buildHoldoutSet(): HoldoutTask[] {
  return [
    ...
    { id: 'holdout-ledger-01', chatId: 1, domain: 'ledger', goal: '汇总三行记账并写入 ledger.csv', artifactKind: 'csv_digest', payload: { answer: 150 } },
    { id: 'holdout-ledger-02', chatId: 1, domain: 'ledger', goal: '汇总退货后净额并写入 ledger.csv', artifactKind: 'csv_digest', payload: { answer: 89 } },
    { id: 'holdout-marker-01', chatId: 1, domain: 'marker', goal: '按要求写入带标记的 report.txt', artifactKind: 'text_marker', payload: { answer: 7 } },
    { id: 'holdout-marker-02', chatId: 1, domain: 'marker', goal: '按要求写入带标记的 report.txt(第二组)', artifactKind: 'text_marker', payload: { answer: 13 } },
  ];
}
