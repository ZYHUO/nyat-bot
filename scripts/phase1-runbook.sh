...
verdict() {
  ...
  # **基线从对照组动态读，不写死**：第一版印"~28%"，那是我早期某轮看到的数；
  # 它下面打印的对照组是现在算出来的。数值巧合一致，但这是"写死的叙述挨着计算值"
  # ——本会话第 79 轮刚为同一个形状栽过（写死的醒来时间）。
  local base
  base=$(npx tsx scripts/control-baseline.mts show 2>/dev/null | grep -e "${CHAT}" | head -1 | sed -E 's/.*均值 +([0-9]+)%.*/\1/')
  echo "主判据    该群发送率  基线 ${base:-?}% → 60%+"
  cat <<'CRITERIA'
