...
> 状态：Phase 0–4 + AGI-like foundation slice 已上线（v0.5.x）；paired replay 离线评测基础已加入。本文是三个月后的自己/接手人看的——先读这篇，再碰 `src/core/`。
...
| foundation slice | 2026-09-12 | 0089–0106 `cognitive_events` / outbox / scope / debt provenance+history / `agency_runs` / attempts+receipts / world+skill history / prediction dimensions+revision ledger / social event graph+prediction ledger / group norm+relationship revisions / route observations / replay experiments | Core/world scope、事件 append/dedupe/replay、outbox worker/projector、task runtime durable facts、sandbox fail-closed、Reply/Heart/Meta/CodeAct/unified tick workspace projections、Agency usage meter、paired replay ablation evaluator、metadata-only social interaction replay、社会预测误差与修复评估、Group/Person hypothesis as-of 回放、Reply 路由成本/质量窗口；authority/canary 仍关闭 |
...
1. **skill review UI 仍偏薄**：`/skill pending|verify|approve|publish|reject` 已接入主人 DM 的 host 命令，后续可补 callback 按钮、revision diff 和更完整的审批审计。
...
6. **Agency runtime 仍未完全接管主路径**：Reply 文本和 Heart/Meta/pipeline wait 已有默认关闭的 authority bridge（`AGENCY_REPLY_TRANSPORT_ENABLED`、`AGENCY_WAIT_TRANSPORT_ENABLED`），分别以真实 Telegram `messageId`、timing `waitUntil/waitJobId` 作为成功条件，并在失败时禁止 legacy fallback；Core proposal 和 Meta dispatch 决策已有 metadata-only shadow 事实，但其它 Meta 动作仍是 legacy，authority/canary 仍需按 chat、成本和恢复指标单独验收。
7. **事件投影仍是窄切片**：当前自动回填可观察 prediction feedback、创建带 source event 的债务、处理成功 tool callback 的确定性偿还，并把严格 scope 的 `world_change` 写入 entity revision；prediction calibration revision ledger 只记录达到证据门槛后的有界修订，不驱动运行时策略。`social_interaction` 已有 metadata-only 可回放事件图，`SOCIAL_PREDICTION_ENABLED` 打开后可记录 engagement prediction error、沉默结算和 repair evaluation，但尚未做关系/规范长期更新或 repair policy。Self/Person/Group/World 已有只读 hypothesis facade，债务已有 scope/source/text 的确定性 matcher；workspace 另提供 host-owned、有界、默认关闭的 semantic scorer 契约，反证驱动的长期更新仍需后续阶段。
8. **Replay 已可用但默认只读**：`replayCognitiveCorrelation` 只读取事件；只有显式 `applyProjections=true` 才会运行幂等 projection，不等价于完整 AGI 评测。
9. **paired evaluator 只做离线对照**：没有真实 held-out 执行结果时，报告只能称为 engineering check；不要把默认 stub 或单次样本的 delta 当作学习/通用能力证据。
...
11. **task recovery summary 是观测切片，不是自动恢复器**：`getTaskRecoverySummary`/`GET /monitor/api/task-recovery` 能从 durable runtime events 和 host acceptance evidence 重建生命周期、checkpoint、计数和 verified boundary；它不会读取 Redis 任务正文，也不会替 executor 续跑、重试或把 `done` 认作成功。真正的重启注入、holdout 任务和 time-to-recover 仍需真实样本与独立验收。
