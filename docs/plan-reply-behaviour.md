...
| 诉求 | 状态 | 证据 |
|---|---|---|
| 多次回复 | 有闸在拦 | task burst：今天拦 7 次（OBJECTIVE-STATUS 10:26） |
| 说话太应激 | 有稳定读数 | interrupt 四数 39/53、0.3/百条、入站 23634（round 191） |
| 不会用别的 bot | 有闸在拦 | 代发目标不在群 21 次 |
| 难融入话题 | **有免费基线、无健康值** | 0 bigram 重合 76.3%（round 207/212） |
| 前言不搭后语 | **有免费基线、无健康值** | 同上 + wait 率 0.2-0.3%（round 201/202） |

后两条拿到的只是**度量**不是修复：没有健康值，无法判断"该修"还是"本来就该这样"。
健康值需要人判"这样的接话正常吗"——见 `known-issues.md` 第 3 档。（见文末「round 200 补记」）
