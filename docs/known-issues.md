...
| 两个 `vi.mock` 指向不存在的路径 | **2 / 1770**（round 239 量） | round 239 | 它们静默无效（mock 注册后永不命中，测试照绿）。修它们有颠適风险（让测试真失败），而不修也无副作用（logger 真加载只打日志）——B 级，等真出问题再修 |
| 「难融入话题」/「前言不搭后语」怎么量 | **kimi 裁决（round 248）：选 C，量具降级为漂移检测器** | round 248 | 已和 kimi 讨论（用户说不接管）。kimi 原判：76.3% 很可能就是健康值，且指标方向可能整个是反的（<20 字回复的高 bigram 重合更像学舌，而"不搭"的典型死法恰是随机复读）。跑了它的两个免费对照实验，**部分推翻它的预测**：人基线 54.7%（它猜 65-80%，差 22.8pp）；洗牌后 91.5% 比真实 77.5% 高 14pp（真实窗口确实含信息，量具不归零）。但它的核心判断被数据支持：bot 6+ 重合只占 1%，人占 13% → 方向未定成立。**落地：绝对值永不报告，只报 bot-人分布差，异常漂移才报警**；"健康值"让给事故计数（和 1/2/3 号同构：失败计数归零/稳定才算闭环） |
## 第 3 档：需要人拍板的设计选择
...
## 结论：bookkeeping-hooks ③④ 只在"跑 meta 目录"时红——全量绿（round 93）

Round 92 排期的分叉问题，结论出来了：

```
npx vitest run tests/unit/meta      → 2 failed（③④）
npm run test（全量 534 文件 4130 测试）→ 全部 passed
```

**全量绿。所以这是目录级测试隔离问题，不是代码回归。**

而且方向和我原来猜的相反：**不是"别的文件覆盖了 mock"，
而是"单独跑这个目录时缺少某个让 mock 生效的前置"**。
最可能是全量跑时另一个文件先 import 了某个模块，把真函数注册进了
mock 的依赖图；单独跑目录时那条链没建立。

**风险评级：低但不是说没问题。**

- 它不影响 CI（CI 跑全量）
- 但它意味着**任何人用 `vitest run tests/unit/meta` 做快速验证时，
  会看到 2 条假红**——而这正是我最常用的验证方式
  （这个会话大概用过几十次）。
...
## interrupt background：三项修法重评，结论是「暂不修」（round 109）

Round 107 我给了三个选项等你拍板。Round 108 补上分母后自己重评：

| 选项 | 内容 | 重评结论 |
|---|---|---|
| A | 长任务只收 addressed | **不必要**。打断率每百条入站 0.2 条（36/22810），频率本身很低 |
| B | 把 background 速率做成可见指标 | **已做**（round 108：`session-report` 的 Interrupt 段） |
| C | 沉默窗口（N 秒后只收 addressed） | **不必要**，同 A |

**为什么不急着修**：round 107 我被 26 条 bare 计数误导，以为很严重。
有了分母（每百条入站 0.2 条）之后，它是个低频现象。

**但保留观测**：`session-report` 每轮都印这个率。如果哪天它涨到
每百条入站 2+ 条，那才是真该动手的时候——**那个阈值就是触发器**，
比"我觉得该修了"可靠。

**顺带**：僵尸那半（round 102-106）是真 bug，已修+自愈清扫。
这里记的是**非泄漏的那半**，它是行为不是 bug。


---

## 「量过但不立守卫」的基线数字台账（round 137 起）

Round 136 立了第四类处置：**量现状 → 记数字 → 说明为什么不立守卫**。
这些数字如果只活在 commit message 里，下次就找不到、也没法对比。
...
**规律**：有守卫的那些，这里的数字只是**基线参照**，红了看守卫；
没守卫的那些（第一行和第四行），这里的数字**就是唯一观测点**。

**复查时机**：round 137 / 157 / 177（每 20 轮），或者任何一轮觉得「最近是不是在丢 X」时。

| 反斜杠-u 转义字面量（CJK 范围） | src **0**（守卫 `no-unicode-escapes-in-src`） / tests **38**（含 3 个守卫自己的正文字面量，round 173 实测） | round 173 | src 涨到 > 0 就红；tests 那 38 里有 4 处是守卫自报，别当病 |
