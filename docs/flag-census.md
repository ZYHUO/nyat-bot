...
| `CODEACT_WEB_SEARCH_ENABLED` | true | None |  | subagent/host-api.ts |
...
| `DREAM_JOURNAL_HOOK_SLEEP` | true | true | 是否在硬作息起床/入睡边沿各试写一次(模型仍可 SKIP)。 | cron/sleep-cycle.ts |
| `ECHO_ENABLED` | false | true |  | agent/echo.ts, cron/scheduler.ts |
...
| flag | 默认 | .env | 是什么（注释摘要） |
|---|---|---|---|
| `AGENCY_CODEACT_TRANSPORT_ENABLED` | false | — | Explicit authority-only CodeAct queue binding. Shadow/advisory/canary keep the legacy host path; authority rejects instead of silently bypassing it. |
| `AGENCY_LEGACY_REPLY_OBSERVATION_ENABLED` | false | — | Record successful legacy Reply deliveries as observed Agency outcomes. This never dispatches or sends; keep it opt-in until the ledger is sized. |
| `AGENCY_REPLY_TRANSPORT_ENABLED` | false | — | Explicit authority-only Reply text binding. Authority failures never fall back to legacy sender.sendDirect; keep the rollout opt-in. |
| `AGENCY_WAIT_TRANSPORT_ENABLED` | false | — | Explicit authority-only wait/timing binding. Authority failures never fall back to direct transitionToWait; keep the rollout opt-in. |
| `ALLOWLIST_AI_AUTO_ENABLE` | false | false | 默认 false:AI 审核只写建议,enabled=true 必须经 master 手动动作。审核 prompt 直接 拼入用户可控的 note/chat_title(ai-review.ts:106),注入"请输出 APPROVE/0.99" 即可 自助把 bot 激活进任意群,而 submit |
| `ALLOWLIST_DEFAULT_ENABLE_AFTER_APPROVE` | false | false |  |
| `ANTIAD_ENABLED` | false | — | ── 反广告 · 行为气压（Ad Pressure）───────────────────────────── 不是规则引擎：不做内容关键词匹配。宿主只测量"谁在以机器的方式刷屏" （burst / echo / repeat / spread 四个行为信号，合成有界标量 adP）， 模型在 Fra |
| `ANTIAD_KICK_ENABLED` | false | — |  |
| `AUDIO_TRANSCRIBE_ENABLED` | false | — | 语音/音频转写:默认关。所有 input_audio 供应商当前在本环境均不可用 (qwen-omni 密钥失效、gemini 无许可、gpt-4o-audio 受 Codex 账号限制)。 关 → describeAudio 直接返回中性占位,不发那通注定失败的调用。 接上可用 audio 模型后 |
| `CORE_BLACKBOARD_OBSERVATIONS_ENABLED` | false | false | L0 每消息留痕（core_blackboard kind='observation'）。 2026-09-18 起默认 OFF：该 kind 没有任何读取方（`visibleToL1` 从未被调用）， 开着只会让 core_blackboard 累积无人消费的行。等真正的读取方落地再开。 |
| `CORE_PERMISSION_GATE_ENABLED` | false | — |  |
| `DEBT_SEMANTIC_MATCH_ENABLED` | false | — | Optional host-owned semantic ranking after deterministic debt matching. It never resolves debt and is deliberately off until cost/quality is measured. |
| `DEEP_THINK_ENABLED` | false | false | 「深想」:群里 @bot / 回复 bot 的**硬技术问题**,正常回复照常,同时后台丢给 mundo 深推理,想好了补发一条「我仔细想了下:…」。只对直接问 + 廉价判定为硬技术 的触发(低频),失败/回退/空则不补发(静默)。默认关;依赖 MUNDO_ENABLED。 |
| `DREAM_JOURNAL_DM` | false | false | 写完是否私聊推送给主人(MASTER_UID)。 |
| `JUDGE_KNOWLEDGE_ENABLED` | false | — |  |
| `MEMORY_DEDUP_ENABLED` | false | — | 写入侧近重复合并:命中已有近邻时不新增点,改为顶高它的 ref_count (「这件事又被说了一次」语义上是强化,不是复制)。压制「哈哈哈」「+1」这类刷屏。 **阈值必须在换完嵌入模型之后标定** —— 旧的英文单语模型下中文相似度普遍虚高 (无关句对都有 0.72),0.93 在旧向量空间里会命 |
| `MEMORY_HYBRID_ENABLED` | false | — | 混合检索:向量召回 + FTS5 BM25 词法召回,按 RRF(名次融合)合并。 384 维小模型对专有名词/群内黑话/型号天然弱(jargon-miner 挖的正是这类词), BM25 补的就是这一块。关闭时完全走旧的纯向量路径。默认关。 |
| `MULTI_AGENT_CRITIC_ON_LOOKUP` | false | false |  |
| `MULTI_AGENT_ENABLED` | true | false | ── Multi-Agent 协调(Orchestrator + 专家 + Writer)── 把"一个 agent 拿所有工具"拆成"几个专职专家并行 + Writer 收口"。 Router 复用 judge.replyPath(direct→chat 跳过专家,planned→lookup/d |
| `MULTI_AGENT_ROUTE_CONVERGENCE_ENABLED` | false | — | Route-convergence experiment: for an explicit allowlist, direct/fast replies stop spawning chat specialists; deep/lookup keep only work justified by t |
| `MUNDO_ENABLED` | false | false | ── Mundo「难题攻坚」部门(可选,默认关)──────────────────────────────── 第三方自建端点上的深推理模型(qwen3.6/映射 Mundo AI),擅长硬算法/并发/调试, 但延迟高、极耗 token、可能空转、端点自签证书不稳定 —— 只适合离线非关键任务且  |
| `NYATDB_REDIS_MIRROR` | false | false |  |
| `NYATDB_VERIFY_ON_OPEN` | false | false |  |
| `PDF_VISION_ENABLED` | false | — | PDF 识别:同理默认关。当前 vision 路由实际落到 GPT(sub2gpt54mini), 读不了 PDF base64,这通调用必败。gemini/PDF-capable vision 恢复后置 true。 |
| `PROACTIVE_COORDINATOR_ENABLED` | false | — | ── P2-A: 主动搭话统一调度 ── 防止 idle + proactive-scan 同时对同一群发消息；全局每群每小时上限 |
| `PROACTIVE_MEMORY_ENABLED` | false | — | ── P2-A: 主动搭话记忆驱动 ── 主动发言时搜索 Qdrant 群聊记忆，注入"上次聊过的相关话题" |
| `REPLY_JSON_MODE` | false | false | 回复写手强制合法 JSON(DeepSeek/OpenAI json_object)——根治单引号/Python-dict 脏输出。默认关。 |
| `STEPFUN_CONSUMER_ENABLED` | false | false | ── StepFun 配额消费引擎(用户选:滚动深反思)────────────────────────── 专用后台引擎:持续对全量群做大窗口深反思 + 跨上下文画像合并,把 8000M/月订阅 用起来(冲 ~100M/天)。默认关。日调用数 ≈ CALLS_PER_TICK × 1440(每分钟 |
...
| key | 默认 | .env | 是什么（注释摘要） |
|---|---|---|---|
| `ADMIN_CORS_ORIGINS` |  | https://miniapp.gomami.wiki | Admin |
...
| `ALLOWLIST_MAX_SUBMISSIONS_PER_DAY` | 20 | 20 |  |
| `ALLOWLIST_REDIS_PREFIX` | 'xxb:mal:' | xxb:mal: |  |
| `ANTIAD_CHAT_IDS` | '' | — |  |
| `ANTI_REPEAT_THRESHOLD` | 0.85 | — |  |
| `ARTIST_USAGE` | 'reply' | artist | 画摊子（agent/artist.ts）的 AI usage 名：SVG 是代码活，默认跟 reply 主链。 |
| `ASI_SAMPLE_RATE` | 0.2 | 0.2 | ASI 回复自评抽样率:1.0 = 全量(每条回复都自评),0.5 = 抽一半。 默认 0.2。ASI rubric 与 realtime-learn 的回复自评对**同一对** (trigger, reply) 各打 一次分,维度都是"贴 |
| `BELIEF_TTL_DEFAULT_SEC` | 7776000 | — |  |
| `BELIEF_VIEW_INJECT_MAX` | 4 | — | Belief View 注入 prompt 的预算（Phase 2 才用，Phase 0 只定义） |
| `BEST_OF_N_BASE` | 1 | 4 | ── AGI Level 6 Phase 15: 小模型增强 ──────────────────────────────── best-of-N 采样基数(按难度翻倍)。verifier 用 judge usage 打分选最优。 |
| `BOT_COMMAND_LEARN_INTERVAL_MIN` | 30 | 30 | 学习扫描间隔(分钟) |
| `BOT_COMMAND_LEARN_USAGE` | 'summarize' | summarize | 学习侧(把观察到的命令提炼成用法/场景)的 LLM 路由。离线 cron、不赶时间、是深 推理任务 → 正好交给 mundo(qwen3.6);设 'mundo' 需 MUNDO_ENABLED。默认走 summarize。 |
| `BOT_DELEGATION_COOLDOWN_SEC` | 60 | 60 | 每群代发限速(秒):两次代发最小间隔 |
| `BOT_NICKNAMES` |  | 啾咪囝,啾咪 |  |
| `BOT_REPLY_DELEGATION_COOLDOWN_SEC` | 60 | 60 | 两次回复式代发最小间隔(秒)。群管动作连着来就像机器在干活。 |
| `BOT_REPLY_DELEGATION_MAX_PER_HOUR` | 3 | 3 | 每群每小时回复式代发上限。超过就只观察不动手。 |
| `BOT_TOKEN` |  | 8392759490:AAGBDKIKf9tlJ-PfZSHznuu2p2v0u | Telegram |
| `BOT_USERNAME` | 'xxb_bot' | hunhebi_bot |  |
| `CACHE_WARMUP_INTERVAL_MIN` | 4 | — |  |
| `CHANNEL_SOURCE_IDS` |  | — | Channel source IDs — channel posts from these channels are ingested into ChromaDB as knowledge |
| `CHANNEL_SOURCE_USERNAMES` |  | zaihuapd | Public channel usernames to scrape (no admin needed, uses t.me/s/ web page) |
| `CODEACT_BANNED_WORDS` |  | — | CodeAct 禁词(逗号分隔),出站文本命中则拒发并要求重写。 |
| `CODEACT_CONCURRENCY` | 4 | — | CodeAct BullMQ / local pump 全局并发；同 chat 仍串行（Redis active lock）。 |
| `CODEACT_MAX_TURNS` | 8 | — |  |
| `CODEACT_TIMEOUT_MS` | 30_000 | 45000 |  |
| `CODEACT_USAGE` | 'reply' | reply |  |
| `COGNITIVE_CONTINUITY_INTERVAL_SEC` | 60 | 60 |  |
| `COGNITIVE_KERNEL_CHAT_IDS` |  | — | Kernel shadow graylist. Empty = all chats once the flag is on; set a few internal chatIds so the first rollout stays mea |
| `COGNITIVE_ROUTING_CHAT_IDS` |  | — |  |
| `COMMON_API_KEY` |  | 74be0b744820a1501488f99b558f71d61c199f56 |  |
| `CONTEXT_MAX_LENGTH` | 600 | 400 |  |
| `CORE_DRIVE_SATIATION_HALFLIFE_SEC` | 21600 | — | Phase 6：drive satiation 半衰期（秒，默认 6h，与 norms TTL 同量级）。 halflifeSec() 经 env() 读这里（Phase 6 前直读 process.env，已收敛）。 |
| `CORE_V2_CHAT_IDS` | '' | — | 灰度群，逗号分隔。空 = 全量生效（与 TURN_ACTOR/MULTI_AGENT 一致）。 |
| `DAILY_LIFE_PROFILE` | 'auto' | auto |  |
| `DEBT_SEMANTIC_MATCH_MAX_CANDIDATES` | 4 | — |  |
| `DEBT_SEMANTIC_MATCH_MIN_SCORE` | 0.72 | — |  |
| `DEBT_SEMANTIC_MATCH_TIMEOUT_MS` | 2_500 | — |  |
| `DEBT_SEMANTIC_MATCH_USAGE` | 'judge' | — |  |
| `DEBT_SWEEP_INTERVAL_MIN` | 30 | — |  |
| `DISTILL_USAGE` | 'summarize' | — | ── AGI Level 4 P4-A: 经验沉淀（常驻）───────────────────────────────── 任务终态复盘蒸馏成 episode + 可复用经验；开工前按 contentDirection 检索相关经验注入  |
| `DM_GREET_AFFINITY_MIN` | 40 | — |  |
| `DM_GREET_MAX_USERS` | 2 | — |  |
| `DM_PROACTIVE_COOLDOWN_HOURS` | 20 | — |  |
| `DREAMING_CRON` | '17 19 * * *' | — | dreaming cron（UTC）。默认 19:17 UTC = 北京 03:17。 |
...
| `EXPRESSION_INJECT_COUNT` | 5 | 3 |  |
| `FETCH_GATEWAY_URL` |  | — |  |
| `FETCH_WORKER_URL` |  | — |  |
| `FIRECRAWL_API_KEY` |  | self-hosted | Firecrawl 兜底:JS 重页面 / Cloudflare 验证页,免费路由(直连/Jina/本地绕过) 全失败后才落到这条付费路由。未配 KEY → 默认关,不发任何 Firecrawl 调用。 |
| `FIRECRAWL_API_URL` | 'https://api.firecrawl.dev' | http://127.0.0.1:3002 |  |
| `GEMINI_API_KEY` |  | AIzaSyDxs8XRVUzn2-HqA6x73L97qjxXc-qodJo | Gemini 联网搜索(Google Search grounding,AI Studio key)。配 KEY 即为主搜索路由。 注:3.1-flash-lite 的 grounding 在免费 key 上 quota=0(需计费);2. |
| `GEMINI_SEARCH_MODEL` | 'gemini-2.5-flash-lite' | gemini-2.5-flash-lite |  |
| `GEMINI_SEARCH_PROXY` |  | http://127.0.0.1:1081 | 本机真实出口地区不支持 grounding(400 User location not supported);设代理只让 Gemini 搜索这一路走代理(其余流量直连,免得 Redis/Qdrant/Firecrawl 等本地连接被绕)。 |
| `GLOBAL_FETCH_PROXY` |  | http://127.0.0.1:1081 | KVM 等受限网络：设 GLOBAL_FETCH_PROXY 后，所有外网 fetch 经 undici ProxyAgent 走代理（Telegram Bot API / LLM / Gemini / web-fetch），本地地址自动直 |
| `GOAL_MAX_ACTIVE` | 20 | 20 | ── AGI Level 4 P4-B: 好奇心目标追踪（常驻）─────────────────────────── 把「值得持续关注的事」固化为 goal，unified-tick 周期性 CodeAct 查进展并汇报。 |
| `GROUNDING_ASKED_MAX` | 0.35 | 0.35 |  |
| `GROUNDING_PRESENT_MAX` | 0.35 | 0.35 | topic_present 低于此值算"聊天里没提过"，user_asked 低于此值算"用户没在问"；两者都低才拦。 |
| `GROUNDING_USAGE` | 'judge' | reflection | grounding 搜索综合用的 AI usage 名（便宜快模型）。 |
| `GROUP_NORMS_INFER_USAGE` | 'judge' | judge |  |
| `GROUP_NORMS_TTL_HOURS` | 6 | 6 |  |
| `HEDGE_DELAY_MS` | 2000 | 0 | AI tuning |
| `HOBBY_DISTILL_USAGE` | 'summarize' | — |  |
| `HOST` | '0.0.0.0' | 0.0.0.0 |  |
| `IP_QUALITY_API_URL` |  | — |  |
| `JARGON_INFERENCE_THRESHOLDS` | '3,8,25,100' | — | G1: 首档 4→3,黑话冷启动更快过推断线(重检计数修复后才有意义) |
| `JUDGE_PROACTIVE_MIN_INTERVAL_SEC` | 120 | 60 |  |
| `JUDGE_PROACTIVE_MIN_RECENT_MSGS` | 3 | — |  |
| `JUDGE_PROACTIVE_RATE` | 0.25 | 0.5 |  |
| `JUDGE_SUBSTRATE_BACKEND` | 'typesafe' | typesafe |  |
...
| `JUDGE_WINDOW_SIZE` | 10 | 30 |  |
| `KNOWLEDGE_BASE_DIR` | './data/knowledge' | — | Knowledge base (file-backed, PHP parity) |
| `KNOWLEDGE_CRON_CHAT_IDS` |  | — | Knowledge cron (cron_long_term.php parity) |
| `KNOWLEDGE_CRON_HASH_PATH` |  | — |  |
| `KNOWLEDGE_CRON_SCHEDULE` | '30 * * * *' | — |  |
| `LEARNER_BATCH_SIZE` | 80 | — |  |
| `LEARNER_MAX_CHATS_PER_TICK` | 3 | 3 |  |
| `LEARNER_MIN_NEW_MSGS` | 30 | 30 |  |
| `LEARNER_SCAN_INTERVAL_MIN` | 60 | 60 |  |
| `LEARNER_SCAN_USAGE` | 'judge' | — |  |
| `LOG_LEVEL` | 'info' | info |  |
| `LOOP_POLICY_MAX` | 5 | 5 |  |
| `MASTER_UID` | 0 | 6251541967 | Business |
| `MASTER_UID_EXTRA` |  | — |  |
| `MEMORY_COLLECTION` | 'xxb_group_history' | xxb_group_history_v2 |  |
| `MEMORY_DEDUP_THRESHOLD` | 0.93 | — |  |
| `MEMORY_EMBED_MODEL` | 'Xenova/all-MiniLM-L6-v2' | Xenova/paraphrase-multilingual-MiniLM-L1 | ── 长期记忆嵌入模型 / collection / 相关性下限 ────────────── 默认的 all-MiniLM-L6-v2 是**英文单语**模型,而本 bot 是中文群聊。生产机实测中文 同义 0.7543 / 无关 0.6 |
| `MEMORY_MIN_SCORE` | 0 | — | 检索相关性下限(0..1)。0 = 不过滤,保持历史行为(纯 topK)。 换模型与调阈值刻意分成两次改动;标定必须用真实语料,别沿用旧模型下的经验值。 |
| `MEMORY_SENSITIVE_CHAT_IDS` |  | — | 始终视作私密的会话(逗号分隔 chatId;群为负数)。DM 由 DM_AUTO_PRIVATE 自动判定。 |
| `MEMORY_STALE_AFTER_DAYS` | 90 | 90 |  |
| `META_ATTENTION_TOP_N` | 8 | 8 | 单次 Meta flush 最多处理几个 attention 条目。 |
| `META_HEART_BYPASS_CHAT_IDS` |  | — | Nyat Trench Phase 1 旁路的**灰度群列表**。空 = 不旁路任何群（默认）。 为什么需要灰度：翻旗的预期效果是把最忙群的发送率从 22% 抬到 86%（投影 3.9x）， 全量翻等于同时改所有群的行为，出了问题也分不清是 |
| `META_HEART_REFRACTORY_MS` | 45_000 | 30000 |  |
| `META_L0_COALESCE_MS` | 2800 | 2800 |  |
| `META_SUBAGENT_CHAT_IDS` |  | — | 灰度 chatId 列表(逗号分隔)。空 = META_SUBAGENT_ENABLED 时对所有 chat 生效。 |
| `META_TICK_MS` | 5000 | 5000 | Meta tick 间隔(ms)。对齐 CGM Attention flush 窗口量级。 |
| `META_USAGE` | 'judge' | judge | Meta / CodeAct 用的 AI usage 名(走现有 AI_USAGE_* 路由)。 |
| `MONITOR_TOKEN` | '' | xxb2026monitor | Monitor |
| `MOOD_DECAY_RATE_PER_HOUR` | 0.3 | — | 每小时衰减比例 (0..1)。0.3 = 1 小时后保留 70% 强度 |
| `MOOD_INJECT_THRESHOLD` | 20 | 20 | \|valence\| < 该阈值时不注入 prompt（默认 calm 不打扰） |
| `MTM_CHUNK` | 150 | — | 每轮压缩的最老消息条数 |
| `MTM_INPUT_MAX_CHARS` | 16000 | — | 压缩输入字符上限(防超长撑爆 summarize 模型) |
| `MTM_MAX_SUMMARIES` | 10 | — | 摘要 FIFO 上限(超出丢最老的) |
| `MULTI_AGENT_CHAT_IDS` |  | — | 灰度群列表(逗号分隔 chatId)。空 = 对所有群生效;非空 = 仅列出的群走多智能体。 |
| `MULTI_AGENT_CHECKER_TIMEOUT_MS` | 10000 | — |  |
| `MULTI_AGENT_CONTEXT_DIGEST_MIN_MSGS` | 12 | — |  |
| `MULTI_AGENT_CONTEXT_DIGEST_TIMEOUT_MS` | 8000 | — |  |
| `MULTI_AGENT_CRITIC_MAX_ROUNDS` | 2 | 2 |  |
| `MULTI_AGENT_CRITIC_TIMEOUT_MS` | 8000 | — |  |
| `MULTI_AGENT_DIRECTOR_TIMEOUT_MS` | 5000 | — |  |
| `MULTI_AGENT_PERSONA_CRITIC_TIMEOUT_MS` | 6000 | — |  |
| `MULTI_AGENT_RESEARCHER_MAX_STEPS` | 6 | — |  |
| `MULTI_AGENT_RESEARCHER_TIMEOUT_MS` | 20000 | — | 专家超时预算(与 turn 打断信号合并;超时→该专家 failed→Writer 回退内部 planner) |
| `MULTI_AGENT_ROUTE_CHAT_IDS` |  | — |  |
| `NODE_ENV` | 'development' | production |  |
| `NO_ACTION_BACKOFF_CAP_SEC` | 300 | 60 |  |
| `NO_ACTION_BACKOFF_START_COUNT` | 2 | — | no_action 指数退避(MaiBot 借鉴):窗口 = base * 2^max(0, n-START), 即第 START_COUNT+1 次 no_action 起开始翻倍,封顶 CAP;continue/真实 回复清零计数。 |
| `NYATDB_CHAT_RING_MAX` | 200 | 200 |  |
| `NYATDB_MAX_MESSAGES_PER_CHAT` | 5000 | 5000 |  |
| `NYATDB_PATH` | './data/nyatdb' | ./data/nyatdb |  |
...
| `NYATOS_SHADOW_TIMEOUT_MS` | 20_000 | 20000 |  |
| `PERSONA_DIR` |  | — | Persona override directory (per-user {uid}.md / .txt) |
| `PHP_WEBHOOK_URL` |  | — |  |
| `PLANNER_MAX_STEPS` | 4 | — | 循环步数上限（MaiBot MAX_INTERNAL_ROUNDS=10,工具场景 4 够用）。 |
| `PORT` | 3000 | 3001 | Server |
| `POST_TASK_FOLLOWUP_USAGE` | 'judge' | reflection | follow-up 判定用的 AI usage 名（便宜快模型）。 |
| `POST_TASK_WINDOW_MS` | 120_000 | — | 发酵窗口时长(ms)。默认 2 分钟。 |
| `PROACTIVE_HOURLY_MAX_PER_CHAT` | 3 | — |  |
| `PROFILE_MERGE_CHAT_IDS` |  | — | 合并灰度群列表(逗号分隔 chatId,群为负数),空 = 对所有上下文生效。 |
| `PROFILE_MERGE_MAX_UIDS` | 8 | 48 |  |
| `PROFILE_MERGE_STALE_HOURS` | 72 | 4 | （SCRATCHPAD_ENABLED 已移除——工作记忆常驻） C:profile-merge 加频 —— 合并水位线间隔(小时)+ 每 tick 处理人数,调小/调大 直接影响全局画像刷新频率与 token 消耗。 |
| `PROFILE_MERGE_USAGE` | 'summarize' | summarize | 全局画像合并走哪个便宜模型 usage 路由。 |
| `PROFILE_SYNC_BATCH_SIZE` | 20 | 60 | 每 tick 处理多少个"有 pending 消息"的用户画像。默认 20;调大可更快榨干 积压的 pending backlog(有意义的真实工作),也提高 StepFun 消耗。 |
| `PROMISE_CHECK_USAGE` | 'reflection' | reflection | 承诺兜底判定用的 AI usage 名（便宜快模型；LLM 判定非规则引擎）。 |
| `QDRANT_HOST` | '127.0.0.1' | 127.0.0.1 | Qdrant (vector memory) — zod-coerced; a non-numeric QDRANT_PORT now fails validation at startup instead of producing `po |
| `QDRANT_PORT` | 6333 | 6333 |  |
| `QUEUE_CONCURRENCY` | 8 | 8 | Queue |
| `RATE_LIMIT_PER_MIN` | 30 | 60 | Rate limiting |
| `REALTIME_LEARN_TIMEOUT_MS` | 10000 | — |  |
| `RECALL_MAX_EXPERIENCE` | 3 | 3 |  |
| `REDIS_URL` | 'redis://127.0.0.1:6379/0' | redis://127.0.0.1:6379/5 | Redis |
| `REFLECTION_CHATS_PER_TICK` | 20 | 15 |  |
| `REFLECTION_INTERVAL_MIN` | 30 | 10 |  |
| `REFLECTION_USAGE` | 'summarize' | reflection |  |
| `REFLECTION_WINDOW_MSGS` | 250 | 200 |  |
...
| `RSS_MAX_ITEM_AGE_HOURS` | 72 | — | 新条目新鲜度闸（小时）：pubDate 比阈值老的直接丢（仍计 seen 防回潮）； 没日期/解析不了的放行（误杀比漏放糟）。2026-08-24：Opus 4.6 旧闻标题党被端上桌的教训。 |
| `RSS_MONITOR_INTERVAL_MIN` | 30 | 30 |  |
| `RSS_USAGE` | 'summarize' | — | 自动发送时使用的 LLM 路由 |
| `SANDBOX_ALLOWED_COMMANDS` | '' | — |  |
| `SANDBOX_BLOCKED_COMMANDS` | 'rm -rf,shutdown,reboot,mkfs,halt,dd if=,chmod 777' | — |  |
| `SEARXNG_URL` |  | — |  |
| `SELF_HISTORY_INJECT_LIMIT` | 5 | 5 |  |
| `SELF_HISTORY_WINDOW_DAYS` | 30 | — |  |
| `SELF_HISTORY_WINDOW_MIN` | 45 | — | 心流看到的"近况"窗口（分钟）。只影响行为史事实块，不影响对某人的一致性注入。 |
| `SELF_PLAY_COOLDOWN_SEC` | 4 * 3600 | — | 两次 self-play 的最小间隔（tick 内 self_play 动作的冷却否决） |
| `SELF_REFLECT_USAGE` | 'judge' | — | ── AGI Level 4 P4-C: 自我模型（常驻）──────────────────────────────── 每天凌晨复盘自己 24h 的回复表现 → ≤5 条自我认知注入回复 prompt。 |
| `SEMANTIC_DUP_THRESHOLD` | 0.7 | — |  |
| `SILENCE_ALERT_CHAT_ID` | 0 | 6251541967 | 告警目标(owner DM chatId,正数)。0=只打日志不发送。 |
| `SILENCE_ALERT_COOLDOWN_MIN` | 120 | 120 | 同一 chat 两次告警的最小间隔(去重,防刷屏)。 |
| `SILENCE_ALERT_HUMAN_STALE_MIN` | 60 | 60 | 人类最后发言距今超过该分钟数 = 不算活跃(潜水群不告警)。 |
| `SILENCE_ALERT_INTERVAL_MIN` | 5 | 5 | 扫描周期(分钟)。 |
| `SILENCE_ALERT_MAX_PER_RUN` | 5 | 5 | 单轮最多告警几个 chat(防告警风暴)。 |
| `SILENCE_ALERT_THRESHOLD_MIN` | 30 | 30 | bot 最后回复距今超过该分钟数 = 判定沉默。 |
| `SKILLS_DIR` | './data/skills' | — | Tool System |
| `SKILL_CONSOLIDATE_USAGE` | 'judge' | — |  |
| `SKILL_DISTILL_INTERVAL_MIN` | 360 | — |  |
| `SKILL_DISTILL_USAGE` | 'summarize' | — |  |
| `SKILL_MAX_BIG` | 50 | — |  |
| `SLEEP_WAKE_WINDOW_MIN` | 20 | — |  |
| `SOCIAL_ACT_SHADOW_CHAT_IDS` |  | — |  |
| `SQLITE_PATH` | './data/xxb.db' | ./data/xxb.db | SQLite |
| `STEPFUN_CONSUMER_CALLS_PER_TICK` | 30 | 4 |  |
| `STEPFUN_CONSUMER_CONCURRENCY` | 4 | 1 | 并发默认 4:StepFun 账号并发上限=8 且与用户可见的 reply/judge 共享,引擎须留余量 (设过高会 429 拖累实时回复)。 |
| `STEPFUN_CONSUMER_REFLECT_WEIGHT` | 3 | 40 | 群深反思在工作池里的权重(重复入池次数):群内容真实演化、最不浪费,给更高权重。 |
| `STEPFUN_SEARCH_API_KEY` | '' | mCv500Dxe2hrBrWoz4Zo2VivSn5llYY4lWzrMX7Q |  |
| `STEPFUN_SEARCH_BASE_URL` | 'https://api.stepfun.com' | — |  |
| `STEPFUN_SEARCH_CATEGORY` | '' | — |  |
| `STEPFUN_SEARCH_MAX_RESULTS` | 5 | — | stepfun 不认 max_results（恒返回 10 条），所以在客户端切。 |
| `STREAMING_MIN_CHARS` | 50 | 50 |  |
| `STREAMING_MIN_INTERVAL` | 500 | 500 | Streaming |
| `SUBAGENT_MEMORY_CHAT_IDS` |  | — | 灰度名单。**空 = 关闭**,与本仓其他 flag 的「空 = 全量」刻意相反: 这是隐私相关特性,配错的代价不对称 —— 漏开只是没效果,误开是内容外泄。 |
| `SUBAGENT_MEMORY_MAX_CHARS` | 600 | — |  |
| `SUBAGENT_MEMORY_TIMEOUT_MS` | 400 | — | 上下界都要:下界防 `TIMEOUT-50` 变成 0 导致「记忆永远为空且与无命中不可区分」, 上界防有人调大后阻塞 CodeAct(那是生产热路径)。 |
| `SUBAGENT_MEMORY_TOPK` | 3 | — |  |
| `TASK_MAX_ROUNDS` | 6 | — |  |
| `TASK_PROGRESS_KEEPALIVE_MS` | 35_000 | — |  |
| `TASK_PROGRESS_MAX_VISIBLE_UPDATES` | 6 | — |  |
| `TASK_PROGRESS_MIN_INTERVAL_MS` | 30_000 | — |  |
| `TASK_PROGRESS_START_DELAY_MS` | 1_000 | — |  |
| `TIC_PENALTY_INTERVAL_MIN` | 30 | — |  |
| `TIC_PENALTY_MIN_FRACTION` | 0.35 | — |  |
| `TIC_PENALTY_MIN_MESSAGES` | 4 | — |  |
| `TIC_PENALTY_TTL_SEC` | 6 * 3600 | — |  |
| `TIC_PENALTY_WINDOW` | 60 | — |  |
| `TIMER_API_URL` |  | — |  |
| `TIMER_CALLBACK_URL` |  | — |  |
...
| `TIMING_TALK_VALUE` | 1.0 | 0.3 | P1-C talk_value 频率阈值(0..1]:1.0 = 该层关闭(no-op)。<1 时非直接消息需攒 ceil(1/有效值) 条才评一次 gate,未达阈值 → defer 延迟重评;有空闲补偿兜底。 per-chat Redi |
...
| `TTS_MAX_CHARS` | 100 | — | 仅对不超过此字符数的回复转语音(长消息发语音很烦)。 |
| `TTS_VOICE` | 'zh-CN-XiaoxiaoNeural' | — | edge-tts 语音名(中文默认晓晓;也可换 zh-CN-XiaoyiNeural 等)。 |
| `TTS_VOICE_PROBABILITY` | 0.15 | — | 每条满足条件的短回复转语音的概率(0..1)。 |
| `TURN_ACTOR_CHAT_IDS` |  | — | 灰度群列表（逗号分隔 chatId）。空 = TURN_ACTOR_ENABLED 时对所有 chat 生效。 |
| `TURN_EXEC_LOCK_TTL_MS` | 120_000 | — |  |
| `TURN_GATE_DEFER_MAX_REPLAYS` | 1 | — | P0-B defer=延迟重评:同一条消息最多被 defer 重排几次(超限按旧语义静默丢弃)。 |
| `TURN_INTERRUPT_MAX_CONSECUTIVE` | 2 | 2 | 连续打断上限（MaiBot planner_interrupt_max_consecutive_count，默认 0=不打断； 我们默认 2 —— 高速群里第二条新消息也应能掐死陈旧生成,review #6）。 |
| `TURN_INTERRUPT_QUIET_MS` | 1000 | — | 打断后静默期（毫秒），等这波消息发完再重规划（MaiBot 硬编码 1s）。 |
| `TURN_MAX_INTERNAL_ROUNDS` | 4 | — | 回合内内部轮次预算（reply + 自我接话 + 余量；MaiBot 是 10，保守起步）。 |
| `TURN_MULTI_ANCHOR_MAX` | 3 | — | 每回合最多回几个人(多锚点预算上限,direct 也算在内)。注意:多锚点会让 单回合最多跑 N 次心流调用 + 发 N 条回复(L7 成本/速率),靠此值约束。 |
| `TURN_SELF_FOLLOWUP_MAX` | 2 | — |  |
| `TYPESAFE_API_KEY` | '' | apikey_218501f56bb4e8e247ee8af454adfae56 |  |
| `TYPESAFE_ENDPOINT` | 'https://api.typesafe.ai/v1/systemone' | https://api.typesafe.ai/v1/systemone | TypeSafe System One 接入。/v1/systemone；key 是 secret（.env，勿提交）。 |
| `TYPESAFE_MODEL` | 'jev-latest' | jev-latest |  |
| `UNIFIED_TICK_HOUR_END` | 23 | — |  |
| `UNIFIED_TICK_HOUR_START` | 8 | — |  |
| `UNIFIED_TICK_INTERVAL_MIN` | 5 | — | ── AGI Level 5 P5-A: 统一唤醒循环（常驻）─────────────────────────── 决策合并：一次 tick 一次 LLM 决定干什么（关心主人/群冒泡/自玩/查goal/安静）， 执行保留旧 cron 的 |
...
| `WEB_FETCH_USER_AGENT` | 'XXB-WebFetch/1.0' | XXB-WebFetch/1.0 |  |
| `WRITER_BEST_OF_N` | 1 | 1 | Best-of-N 写手:生成 N 稿,选择器挑最贴的发。N=1 关闭。写手 token ×N。 默认 1。best-of-N 对 direct 闲聊路由没有降级(orchestrator.ts:281),等于让一个 maxTokens:2 |
...
| `TTS_VOICE_PROBABILITY` | — | 每条满足条件的短回复转语音的概率(0..1)。 |
| `TURN_UNIFIED_DECISION_ENABLED` | — | (旧名,弃用,留着防 .env 报错) |
