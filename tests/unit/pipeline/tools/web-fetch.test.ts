...
describe('executeFetch + Firecrawl fallback', () => {
  ...
  it('routes nodeseek.com straight to Firecrawl (no pinned fetch, no reader)', async () => {
    // NodeSeek 专用 reader 已删:tryPublicSiteSource 直接走 Firecrawl,不碰直连/reader。
    globalThis.fetch = vi.fn().mockImplementation((u: string) => {
      if (typeof u === 'string' && u.includes('api.firecrawl.dev')) {
        return Promise.resolve({
          ok: true,
          json: async () => ({ success: true, data: { markdown: 'NodeSeek 首页内容', metadata: { title: 'NodeSeek' } } }),
        });
      }
      return Promise.resolve({ ok: false, json: async () => ({ error: 'fail' }), text: async () => '' });
    }) as unknown as typeof fetch;
    ...
    expect(mockFetchUrlPinned).not.toHaveBeenCalled(); // 没走通用直连
    expect(globalThis.fetch).toHaveBeenCalledWith(
      'https://api.firecrawl.dev/v1/scrape',
      expect.objectContaining({ method: 'POST' }),
    );
    expect(result).toContain('NodeSeek 首页内容');
